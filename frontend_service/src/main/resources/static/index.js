angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.decrementInCart = function (productId){
        $http.get('http://localhost:8190/winter-carts/api/v1/cart/decrement/' + productId).then(function (response){
            $scope.loadCart();
        });
    }
    ...
    $scope.clearCart = function (){
        $http.put('http://localhost:8190/winter-carts/api/v1/cart/clear').then(function (response){
            $scope.loadCart();
        })
    }
